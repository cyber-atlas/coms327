...
void do_combat(dungeon *d, character *atk, character *def) {
  ...
  if (atk != d->PC && def != d->PC) { //npcs swap instead of killing each other
	character *temp = def;
  ...
  if (atk == d->PC && character_is_alive(def)) {
	uint32_t atk_dmg = d->PC->damage->roll(); //initiate the dmg with pc's current dmg
	for (int i = 0; i < NUM_EQUIPMENT; ++i) { //add all atk dmg of equipped items
	  if (d->PC->equipped[i]) {
	...
	def->hp -= atk_dmg;
	if (def->hp < 0) { //the defender is now dead
	  --d->num_monsters;
	  character_die(def);
  ...
  if(atk != d->PC && character_is_alive(def) && def == d->PC) {
    ...
    if(def->hp < atk_dmg) { //pc will not survive
      character_die(def);
      character_increment_dkills(atk);
      character_increment_ikills(atk, (character_get_dkills(def)) +
	      character_get_ikills(def));
...
void move_character(dungeon *d, character *c, pair_t next) {
  ...
  if (c == d->PC) {
	if (d->objmap[next[dim_y]][next[dim_x]]) { //if there is an object here
	  for (int i = 0; i < NUM_INVENTORY; ++i) {
		if (d->PC->inventory[i] == NULL) { //check to see if it can be picked up
		  d->PC->inventory[i] = d->objmap[next[dim_y]][next[dim_x]];
		  d->objmap[next[dim_y]][next[dim_x]] = NULL; //remove from map
		}
	  }
	}
