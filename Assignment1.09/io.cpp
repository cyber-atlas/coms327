...
void addto_inv(dungeon *d, int desired_object) {
  int is_full = 1;
  for (int i = 0; i < NUM_INVENTORY; ++i) {
	if (d->PC->inventory[i] == NULL) { //can be placed here
...
/**
 * Wear an item. Prompts the user for a carry slot. If an
 * item of that type is already equipped, items are swapped.
 * @param d dungeon
 * @param invent_slot selected slot in inventory to equip
 */
void wear_equipment(dungeon_t *d, int invent_slot) {
  if (d->PC->inventory[invent_slot]->get_type() == objtype_WEAPON) {
	if (d->PC->equipped[weapon] == NULL) { //there is room for it
	  d->PC->equipped[weapon] = d->PC->inventory[invent_slot];
	  d->PC->inventory[invent_slot] = NULL;
	  organize_inventory(d);
	} else { //swap the inventory object with the equipped object
	...
	if (d->PC->equipped[offhand] == NULL) { //there is room for it
	...
	if (d->PC->equipped[ranged] == NULL) { //there is room for it
	...
	if (d->PC->equipped[armor] == NULL) { //there is room for it
	...
	if (d->PC->equipped[helmet] == NULL) { //there is room for it
	...
	if (d->PC->equipped[cloak] == NULL) { //there is room for it
	...
	if (d->PC->equipped[gloves] == NULL) { //there is room for it
	...
	if (d->PC->equipped[boots] == NULL) { //there is room for it
	...
	if (d->PC->equipped[amulet] == NULL) { //there is room for it
	...
	if (d->PC->equipped[light] == NULL) { //there is room for it
	...
	if (d->PC->equipped[ring_one] == NULL) { //there is room for it
...
void io_display_inv(dungeon_t *d) {
  ...
  switch (int input = getch()) {
	case 'w':mvprintw(20, 5, "Choose an item to equip                                                  ");
	  switch (input = getch()) { //equipping item
		case '0':
		  if (d->PC->inventory[0] != NULL) {
			wear_equipment(d, 0);
		  }
  ...
  switch (input = getch()) {
	case 't': //take off item
	  ...
	  switch (input = getch()) {
		case 'a':
		  if (d->PC->equipped[weapon] != NULL) {
			d->PC->speed -= d->PC->equipped[weapon]->get_speed(); //subtract the bonus
			addto_inv(d, weapon);
		  }
		  break;
		case 'b':
		  if (d->PC->equipped[offhand] != NULL) {
			d->PC->speed -= d->PC->equipped[offhand]->get_speed(); //subtract the bonus
			addto_inv(d, offhand);
		  }
		  break;
		case 'c':
		  if (d->PC->equipped[ranged] != NULL) {
			d->PC->speed -= d->PC->equipped[ranged]->get_speed(); //subtract the bonus
			addto_inv(d, ranged);
		  }
		  break;
		case 'd':
		  if (d->PC->equipped[armor] != NULL) {
			d->PC->speed -= d->PC->equipped[armor]->get_speed(); //subtract the bonus
		addto_inv(d, armor);
		  }
		  break;
		case 'e':
		  if (d->PC->equipped[helmet] != NULL) {
			d->PC->speed -= d->PC->equipped[helmet]->get_speed(); //subtract the bonus
			addto_inv(d, helmet);
		  }
		  break;
		case 'f':
		  if (d->PC->equipped[cloak] != NULL) {
			d->PC->speed -= d->PC->equipped[cloak]->get_speed(); //subtract the bonus
			addto_inv(d, cloak);
		  }
		  break;
		case 'g':
		  if (d->PC->equipped[gloves] != NULL) {
			d->PC->speed -= d->PC->equipped[gloves]->get_speed(); //subtract the bonus
			addto_inv(d, gloves);
		  }
		  break;
		case 'h':
		  if (d->PC->equipped[boots] != NULL) {
			d->PC->speed -= d->PC->equipped[boots]->get_speed(); //subtract the bonus
			addto_inv(d, boots);
		  }
		  break;
		case 'i':
		  if (d->PC->equipped[amulet] != NULL) {
			d->PC->speed -= d->PC->equipped[amulet]->get_speed(); //subtract the bonus
			addto_inv(d, amulet);
		  }
		  break;
		case 'j':
		  if (d->PC->equipped[light] != NULL) {
			d->PC->speed -= d->PC->equipped[light]->get_speed(); //subtract the bonus
			addto_inv(d, light);
		  }
		  break;
		case 'k':
		  if (d->PC->equipped[ring_one] != NULL) {
			d->PC->speed -= d->PC->equipped[ring_one]->get_speed(); //subtract the bonus
			addto_inv(d, ring_one);
		  }
		  break;
		case '1':
		  if (d->PC->equipped[ring_two] != NULL) {
			d->PC->speed -= d->PC->equipped[ring_two]->get_speed(); //subtract the bonus
			addto_inv(d, ring_two);
		  }
		  break;
		default: break;
	  }
...
/**
 * Inspect an item. Prompts user for a carry slot. Item's
...
/**
 * Look at a monster. Enter a targeting mode similar to the
 * controlled teleport in 1.06. Select a visible monster with t
 * or abort with esacpe (there is no random). When a monster is
 * selected, display its description (and other information, if
 * you like). Escape will return back to normal input processing.
 * @param d dungeon
 */
